# ==============================================================================
# corefinance - Go Backend Makefile
# ==============================================================================
#
# Prerequisites:
# - Go 1.21+
...
#   postgresql://neondb_owner:...@<direct-host>/coredb?sslmode=require&channel_binding=require&options=-c%20search_path%3Dcorefinance
...
help:
	@echo ""
	@echo -e "${BLUE}corefinance - Available Commands${NC}"
	@echo ""
...
run:
	@echo -e "${YELLOW}Starting corefinance in development mode...${NC}"
	@ENV=$(ENV) PORT=$(PORT) go run ./cmd/api

dev:
	@echo -e "${YELLOW}Starting corefinance with live reload...${NC}"
...
build:
	@echo -e "${YELLOW}Building corefinance...${NC}"
	@mkdir -p $(BUILD_DIR)
	@go build -o $(BUILD_DIR)/$(BINARY_NAME) -ldflags="-s -w" ./cmd/api
	@echo -e "${GREEN}Built successfully: $(BUILD_DIR)/$(BINARY_NAME)${NC}"

build-docker:
	@echo -e "${YELLOW}Building Docker image...${NC}"
	@docker build -t corefinance:latest .
...
start:
	@echo -e "${YELLOW}Starting corefinance...${NC}"
	@ENV=$(ENV) PORT=$(PORT) ./$(BUILD_DIR)/$(BINARY_NAME)

run-background:
	@echo -e "${YELLOW}Starting corefinance in background...${NC}"
	@nohup ENV=$(ENV) PORT=$(PORT) ./$(BUILD_DIR)/$(BINARY_NAME) > api.log 2>&1 &
	@echo -e "${GREEN}corefinance started in background (PID: $$!)${NC}"
