...
--   - DATABASE_URL  = -pooler host, corefinance_app, no URL params
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'corefinance_app') THEN
    CREATE ROLE corefinance_app LOGIN PASSWORD 'REPLACE_ME';
  END IF;
END
$$;

GRANT USAGE ON SCHEMA corefinance TO corefinance_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA corefinance TO corefinance_app;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA corefinance TO corefinance_app;
...
ALTER DEFAULT PRIVILEGES FOR ROLE neondb_owner IN SCHEMA corefinance
  GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO corefinance_app;
ALTER DEFAULT PRIVILEGES FOR ROLE neondb_owner IN SCHEMA corefinance
  GRANT USAGE, SELECT ON SEQUENCES TO corefinance_app;

-- Pooler-safe schema scoping for the runtime role. No search_path / options
-- params needed in DATABASE_URL.
ALTER ROLE corefinance_app SET search_path = corefinance;
