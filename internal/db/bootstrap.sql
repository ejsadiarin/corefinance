-- ==============================================================================
-- corefinance - one-time bootstrap (NOT a goose migration)
-- ==============================================================================
--
...
--   - DATABASE_URL  = -pooler host, corefinance_role, no URL params
...
-- ownership implies DROP/ALTER, and the API/worker must never hold DDL.
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'corefinance_role') THEN
    CREATE ROLE corefinance_role LOGIN PASSWORD 'REPLACE_ME';
  END IF;
END
$$;

GRANT USAGE ON SCHEMA corefinance TO corefinance_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA corefinance TO corefinance_role;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA corefinance TO corefinance_role;
...
ALTER DEFAULT PRIVILEGES FOR ROLE neondb_owner IN SCHEMA corefinance
  GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO corefinance_role;
ALTER DEFAULT PRIVILEGES FOR ROLE neondb_owner IN SCHEMA corefinance
  GRANT USAGE, SELECT ON SEQUENCES TO corefinance_role;

-- Pooler-safe schema scoping for the runtime role. No search_path / options
-- params needed in DATABASE_URL.
ALTER ROLE corefinance_role SET search_path = corefinance;
