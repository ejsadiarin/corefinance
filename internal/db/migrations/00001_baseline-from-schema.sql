-- +goose Up
-- Baseline capturing services/corefinance/internal/db/schema.sql verbatim
...
-- +goose Down
-- Baseline Down is staging-only: drops everything the Up created, in reverse
-- dependency order. CASCADE covers the foreign keys / dependent objects.
DROP TABLE IF EXISTS expense_tags CASCADE;
DROP TABLE IF EXISTS income_tags CASCADE;
